...
telem_data_for_analysis <- telem_data %>%
  ...
  rename(focal_area_hectares = area) %>%
  ...
  mutate(log_calories_removed_per_ha = log(calories_removed_per_ha + 1),
         log_focal_area_hectares = log(focal_area_hectares)) %>%
  mutate(analysis = "telemetry") %>%
  mutate(subanalysis = "telemetry") %>%
  select(analysis, subanalysis, bird_group, year, patch, year_patch, control_treatment, exp_phase, bird_id, frequency, species, sex, obs_effort_minutes, everything())
```
...
subset_data <- function(dataset, analysis, subanalysis, bird_group){
  ...
  dataset <- dataset %>%
    ...
    mutate(across(c(log_calories_removed_per_ha, log_focal_area_hectares), ~c(scale(.))))
...
path_name <- c("results/statistical_analysis/assumption_checks/telemetry")
...
```{r}
saveRDS(results, "results/statistical_analysis/rds_files/Telemetry_data_results.rds")
```
