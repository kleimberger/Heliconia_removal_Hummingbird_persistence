...
#Focal area size + Heliconia density
heto_densities <- read.csv("data/export/supp_tables/Heliconia_densities.csv") %>%
  select(year, patch, focal_area_size = area, heto_calories_per_ha = calories_per_ha) %>%
  mutate(log_focal_area_size = log(focal_area_size)) %>%
  mutate(log_heto_calories_per_ha = log(heto_calories_per_ha))

#Combine into one table
site_characteristics <- elevation %>%
  left_join(heto_densities) %>%
...
```{r}
make_plot = function(dataset, yvar, ylabel, size, shape_size, line_width){
  
  plot <- 
  ggplot(data = dataset, aes_string(x = "control_treatment", y = yvar)) +
  
    #Boxplots in different position than points
    geom_boxplot(aes(fill = control_treatment), colour = "black", width = 0.15, position = position_nudge(x = c(-0.15, 0.15)), alpha = 0.5, outlier.shape = NA, size = line_width) +
    stat_summary(fun = mean, position = position_nudge(x = c(-0.15, 0.15)), geom = "point", shape = 18, size = shape_size) +
    geom_point(aes(fill = control_treatment), colour = "black", shape = 21, position = position_dodge(width = 0.45), alpha = 1, size = shape_size, stroke = line_width) +
...
(elevation_plot <- make_plot(dataset = site_characteristics, yvar = "elevation", ylabel = "Elevation (m)", size = size, shape_size = shape_size, line_width = line_width))
(focal_area_plot <- make_plot(dataset = site_characteristics, yvar = "focal_area_size", ylabel = "Focal area size (ha)", size = size, shape_size = shape_size, line_width = line_width))
(heto_density_plot <- make_plot(dataset = site_characteristics, yvar = "log_heto_calories_per_ha", ylabel = expression("Ln"~italic(Heliconia)~"calories/ha"), size = size, shape_size = shape_size , line_width = line_width))
...
ggsave(file.path(path_name, "FigSX_Site_characteristics.png"), site_characteristic_multiplot, dpi = 300, width = column2x, height = column2x/3, units = "px", bg = "white", device = "tiff")
```
