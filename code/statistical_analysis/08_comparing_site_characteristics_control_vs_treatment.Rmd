...
make_plot = function(dataset, yvar, ylabel, text_size, point_size, line_size){
  ...
  ggplot(data = dataset, aes_string(x = "control_treatment", y = yvar)) +
    ...
    geom_boxplot(aes(fill = control_treatment), colour = "black", width = 0.15, position = position_nudge(x = c(-0.15, 0.15)), alpha = 0.5, outlier.shape = NA, size = line_size) +
    stat_summary(fun = mean, position = position_nudge(x = c(-0.15, 0.15)), geom = "point", shape = 18, size = point_size) +
