...
#Model summaries
summary(model_all_spp)
summary(model_greh_visa)

#Estimated marginal means
#Condition argument (sightings = 1) gives the average sighting length (i.e., estimated amount of time, in seconds, for 1 sighting)
(ggeffects_all_spp <- ggemmeans(model_all_spp, terms = c("exp_phase", "control_treatment"), back.transform = TRUE, condition = c(sightings = 1)) %>%
  as.data.frame() %>%
  mutate_if(is.numeric, ~round(., digits = 2)))

(ggeffects_greh_visa <- ggemmeans(model_greh_visa, terms = c("exp_phase", "control_treatment"), back.transform = TRUE, condition = c(sightings = 1)) %>%
  as.data.frame() %>%
  mutate_if(is.numeric, ~round(., digits = 2)))
```

**Plot results (interaction plots)**
```{r, fig.width=3, fig.height=4.5, fig.align='center'}
...
#Plots
plot_all_spp <- make_interaction_plot(ggeffects_df = ggeffects_all_spp, yvar = "visit_duration", ymin = 5, ymax = 10, ybreak = 1, yaccuracy = 1, text_size = size, point_size = shape_size, line_size = line_width, ylabel = "Visit duration (seconds)")

# ggview::ggview(plot_all_spp, width = column1x, height = column1x, units = "px")

plot_greh_visa <- make_interaction_plot(ggeffects_df = ggeffects_greh_visa, yvar = "visit_duration", ymin = 5, ymax = 10, ybreak = 1, yaccuracy = 1, text_size = size, point_size = shape_size, line_size = line_width, ylabel = "Visit duration (seconds)")

# ggview::ggview(plot_greh_visa, width = column1x, height = column1x, units = "px")
...
multiplot
# ggview::ggview(multiplot, width = column1x, height = column1x*1,5, units = "px")
```
