...
model_fitting_nonheto_separate %>% filter(plant_species == "COSTBAR") %>% select(-model_name) %>% unnest(fit_with_info) %>% pull(dharma_plot) #any (m1 lower AIC)
...
model_fitting_nonheto_separate %>% filter(plant_species == "PAV-ROJ-AMA") %>% select(-model_name) %>% unnest(fit_with_info) %>% pull(dharma_plot) #any (m3 lower AIC but all values <2)
