...
**Remove plant species that had no sightings from any hummingbird species, pre or post**
```{r}
##1. Combine data
...
**Heliconia visitation rate: all birds, including marked birds**
...
**Non-Heliconia visitation rate (all plant species together): all birds, including marked birds**
```{r, include = FALSE, eval = FALSE}
fit_data02 <- data %>%
...
**Non-Heliconia visitation rate (plant species separate): all birds, including marked birds**
...
fit_model <- function(dataset, plant_species){
  ...
  fit_data <- dataset %>%
    filter(analysis == "camera_non-HETO" & subanalysis == "all_birds" & model_type == "categorical") %>%
  ...
  m2 <- glmmTMB::glmmTMB(sightings ~ control_treatment * exp_phase + flowers + offset(log(hours)) + (1|patch/year_patch), data = fit_data, na.action = na.omit, family = "poisson", ziformula = ~1)
  ...
  m4 <- glmmTMB::glmmTMB(sightings ~ control_treatment * exp_phase + flowers + offset(log(hours)) + (1|patch/year_patch), data = fit_data, na.action = na.omit, family = "nbinom1", ziformula = ~1)
  ...
  m6 <- glmmTMB::glmmTMB(sightings ~ control_treatment * exp_phase + flowers + offset(log(hours)) + (1|patch/year_patch), data = fit_data, na.action = na.omit, family = "nbinom2", ziformula = ~1)

  #Combine models
  model_fitting <- tibble(model_name = c("m1", "m2", "m3", "m4", "m5", "m6"), model = list(m1, m2, m3, m4, m5, m6)) %>%
    mutate(dharma_object = map(model, ~simulateResiduals(fittedModel = ., n = 1000, plot = FALSE, re.form = ~0))) %>%
    mutate(dharma_plot = pmap(list(dharma_object, model_name), ~make_dharma_plot(dharma_object = ..1, plot_title = ..2,  plot_type = "basic")),
           dharma_zeroinfl_plot = pmap(list(dharma_object, model_name), ~make_dharma_plot(dharma_object = ..1, plot_title = ..2,  plot_type = "zeroinflation")),
           dharma_overdisp_plot = pmap(list(dharma_object, model_name), ~make_dharma_plot(dharma_object = ..1, plot_title = ..2,  plot_type = "overdispersion"))) %>%
...
#model 3 is best for all except for CEPO (m1), COSTBAR (m1), and BOCA-ROJA (m5 - but AICc is basically the same as m3) 
#will use mmodel 3 (nbinom1) for most species and model 1 (Poisson) for CEPO + COSTBAR
```
