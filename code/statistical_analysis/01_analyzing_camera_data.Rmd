...
**How many non-Heliconia species were had camera data pre and post? Did all species have sightings**
```{r}
...
nonheto_species_sample_size <- nonheto_data %>%
  ...
  select(plant_family, plant_scientific_name, plant_species, num_replicates)

#Export list for supp material
write.csv(nonheto_species_sample_size, "data/export/supp_tables/Non-Heliconia_camera_species_list.csv")
```
...
#Check: how many plant species, including HETO?
unique(visit_data_for_analysis$plant_species) %>% length()
```
...
heto_nonheto_combined_base_table <- crossing(analysis, subanalysis, bird_group, model_type) %>%
  filter(!(subanalysis == "marked_birds" & model_type == "quantitative")) #Looking at quantitative models for these "additional" analyses seems excessive
...
subset_data <- function(dataset, analysis, subanalysis, bird_group, plant_species){
  ...
  if(analysis == "camera_non-HETO_separate"){
    ...
    dataset <- dataset %>%
      filter(analysis == "camera_non-HETO" & plant_species == {{ plant_species }}) %>%
...
fit_model <- function(dataset, plant_species){
  
  fit_data <- dataset %>%
    filter(analysis == "camera_non-HETO_separate" & plant_species == {{ plant_species }}) %>%
  ...
  m1 <- glmmTMB::glmmTMB(sightings ~ control_treatment * exp_phase + flowers + offset(log(hours)) + (1|patch/year_patch), data = fit_data, na.action = na.omit, family = "poisson", ziformula = ~0,
                         control=glmmTMBControl(optimizer=optim, optArgs=list(method="BFGS")))

  m3 <- glmmTMB::glmmTMB(sightings ~ control_treatment * exp_phase + flowers + offset(log(hours)) + (1|patch/year_patch), data = fit_data, na.action = na.omit, family = "nbinom1", ziformula = ~0,
                         control=glmmTMBControl(optimizer=optim, optArgs=list(method="BFGS")))

  m5 <- glmmTMB::glmmTMB(sightings ~ control_treatment * exp_phase + flowers + offset(log(hours)) + (1|patch/year_patch), data = fit_data, na.action = na.omit, family = "nbinom2", ziformula = ~0,
                         control=glmmTMBControl(optimizer=optim, optArgs=list(method="BFGS")))
...
#Only look at species with at least 10 replicates/20 rows 
model_fitting_nonheto_separate <- nonheto_species_sample_size %>%
  filter(num_replicates > 10) %>%
  arrange(plant_species) %>%
  mutate(fit_with_info = map(plant_species, ~fit_model(dataset = data, plant_species = .)))
...
#According to AICc, model 3 is best for all except for BOCA-ROJA (m5 - but AICc is basically the same as m1 and m3), CEPO (m1 - but AICc is <3 from m3 and m5), and COSTBAR (m1)
#But...do best models according to AICc meet assumptions? OR are other models better? Check to make sure assumptions look OK. I've noted best model(s) according to assumptions below.
model_fitting_nonheto_separate %>% filter(plant_species == "BOCA-ROJA") %>% unnest(fit_with_info) %>% pull(dharma_plot) #m1
model_fitting_nonheto_separate %>% filter(plant_species == "CEPO") %>% unnest(fit_with_info) %>% pull(dharma_plot) #m5
model_fitting_nonheto_separate %>% filter(plant_species == "COSTBAR") %>% unnest(fit_with_info) %>% pull(dharma_plot) #any (m1 lower AIC)
model_fitting_nonheto_separate %>% filter(plant_species == "HAPA") %>% unnest(fit_with_info) %>% pull(dharma_plot) #m1, m5 (m5 lower AIC)
model_fitting_nonheto_separate %>% filter(plant_species == "MARA") %>% unnest(fit_with_info) %>% pull(dharma_plot) #m1, m3 (m3 lower AIC)
model_fitting_nonheto_separate %>% filter(plant_species == "PALU") %>% unnest(fit_with_info) %>% pull(dharma_plot) #m3 
model_fitting_nonheto_separate %>% filter(plant_species == "PAV-ROJ-AMA") %>% unnest(fit_with_info) %>% pull(dharma_plot) #any (m3 lower AIC but all values <2)
model_fitting_nonheto_separate %>% filter(plant_species == "RABO") %>% unnest(fit_with_info) %>% pull(dharma_plot) #m3 but still rather squiggly :(
model_fitting_nonheto_separate %>% filter(plant_species == "TUBO-AMA") %>% unnest(fit_with_info) %>% pull(dharma_plot) #m3, m5 (m3 lower AIC)

#Troubleshooting RABO models...
rabo_data <- data %>% filter(plant_species == "RABO") %>% pull(data) %>% as.data.frame()
rabo_fit_zip <- glmmTMB::glmmTMB(sightings ~ control_treatment * exp_phase + flowers + offset(log(hours)) + (1|patch/year_patch), data = rabo_data, na.action = na.omit, family = "poisson", ziformula = ~1, control=glmmTMBControl(optimizer=optim, optArgs=list(method="BFGS")))

rabo_fit_genpois <- glmmTMB::glmmTMB(sightings ~ control_treatment * exp_phase + flowers + offset(log(hours)) + (1|patch/year_patch), data = rabo_data, na.action = na.omit, family = "genpois", ziformula = ~0, control=glmmTMBControl(optimizer=optim, optArgs=list(method="BFGS")))

rabo_fit_flowers2 <- glmmTMB::glmmTMB(sightings ~ control_treatment * exp_phase + I(flowers^2) + offset(log(hours)) + (1|patch/year_patch), data = rabo_data, na.action = na.omit, family = "nbinom1", ziformula = ~0, control=glmmTMBControl(optimizer=optim, optArgs=list(method="BFGS")))

rabo_fit_flowers2_no_offset <- glmmTMB::glmmTMB(sightings ~ control_treatment * exp_phase + I(flowers^2) + hours + (1|patch/year_patch), data = rabo_data, na.action = na.omit, family = "nbinom1", ziformula = ~0, control=glmmTMBControl(optimizer=optim, optArgs=list(method="BFGS")))
...
#DECISION:
#Model 1 (Poisson) for BOCA-ROJA, COSTBAR
#Model 5 (nbinom2) for CEPO, HAPA
#Model 3 (nbinom1) for MARA, PALU, PAV-ROJ-AMA, TUBO-AMA
...
create_model <- function(dataset, analysis, subanalysis, treatment_variable, plant_species){
  ...
  if(analysis == "camera_non-HETO_separate"){
    ...
    #For these two plant species, use Poisson
    if(plant_species == "BOCA-ROJA" | plant_species == "COSTBAR"){
      
      model <- glmmTMB(as.formula(paste0(base_formula, " + (1|patch/year_patch)")), data = dataset, na.action = na.omit, family = "poisson", ziformula = ~0,
                       control=glmmTMBControl(optimizer=optim, optArgs=list(method="BFGS")))
      
    }
    
    #For these four plant species, use nbinom1
    nbinom2_species <- c("MARA", "PALU", "PAV-ROJ-AMA", "TUBO-AMA")
    if(plant_species %in% nbinom2_species){
    ...
    #For these two plant species, use nbinom2
    if(plant_species == "CEPO" | plant_species == "HAPA"){
      
      model <- glmmTMB(as.formula(paste0(base_formula, " + (1|patch/year_patch)")), data = dataset, na.action = na.omit, family = "nbinom2", ziformula = ~0,
                       control=glmmTMBControl(optimizer=optim, optArgs=list(method="BFGS")))
    ...
    if(plant_species == "RABO"){
      ...
      model <- glmmTMB(as.formula("sightings ~ control_treatment * exp_phase + I(flowers^2) + hours + (1|patch/year_patch)"), data = dataset, na.action = na.omit, family = "nbinom1", ziformula = ~0,
                       control=glmmTMBControl(optimizer=optim, optArgs=list(method="BFGS")))
...
# test02 <- create_model(dataset = data[[12]][[13]], analysis = "camera_non-HETO_separate", subanalysis = "all_birds", treatment_variable = "control_treatment", plant_species = "BOCA-ROJA")
# summary(test01)
# summary(test02)
