...
capture_contrast_plot <- contrasts_of_contrasts_df %>%
  ...
  make_contrast_plot(contrasts_df = ., xvar = "bird_group") +
...
flower_visits_contrast_plot <- contrasts_of_contrasts_df %>%
...
flower_visits_contrast_plot

#POLLINATION SUCCESS
pollination_contrast_plot <- contrasts_of_contrasts_df %>%
...
#MARKED BIRDS
marked_contrast_plot <- contrasts_of_contrasts_df %>%
...
captures_multiplot <- ggarrange(plotlist = capture_plots, nrow = 1, ncol = 3, legend = "none", align = "hv", labels = c("(a)", "(b)", "(c)"), font.label = list(size = 18, face = "plain")) %>%
  annotate_figure(top = text_grob("Hummingbird abundance", size = 24, face = "plain", hjust = 0, x = 0.01))
...
telemetry_multiplot <- ggarrange(plotlist = telemetry_plots, nrow = 1, ncol = 3, legend = "none", align = "hv", labels = c("(d)", "(e)", "(f)"), font.label = list(size = 18, face = "plain")) %>%
  annotate_figure(top = text_grob("Hummingbird space use", size = 24, face = "plain",  hjust = 0, x = 0.01))
...
captures_telemetry_multiplot <- ggarrange(captures_multiplot, NA, telemetry_multiplot, legend_arranged, nrow = 4, ncol = 1, heights = c(1, 0.05, 1, 0.05))
ggview::ggview(captures_telemetry_multiplot, width = 14, height = 12, units = "in")
```
...
Figure 6: Additional response variables (recaptures, individual marked birds, relative body mass)
```{r}

```

## BELOW HERE IS OLD AS OF 03/10/2023
