...
interaction_plots <- emmeans_for_plots %>%
  ...
  mutate(interaction_plot = pmap(list(ggeffects, yvar, ymin, ymax, ybreak, yaccuracy), ~make_interaction_plot(ggeffects_df = ..1, yvar = ..2, ymin = ..3, ymax = ..4, ybreak = ..5, yaccuracy = ..6, text_size = size, point_size = shape_size, line_size = line_width, as_percent = TRUE)))
```
...
legend_arranged <- ggarrange(interaction_plot_legend, NA, nrow = 1, ncol = 2, widths = c(1, 0.5)) #This makes the legend centered under the interaction plots instead of under all plots

captures_telemetry_multiplot <- ggarrange(captures_multiplot, NA, telemetry_multiplot, NA, legend_arranged, nrow = 5, ncol = 1, heights = c(1, 0.05, 1, 0.05, 0.05))
ggview::ggview(captures_telemetry_multiplot, width = column1.5x, height = column1.5x*0.75, units = "px")
ggsave(file.path(path_name, "Fig3_Captures_and_Telemetry.png"), captures_telemetry_multiplot, dpi = 300, width = column1.5x, height = column1.5x*0.75, units = "px", bg = "white", device = "tiff")
```
...
flower_visits_multiplot <- ggarrange(heto_visits_multiplot, NA, nonheto_visits_multiplot, NA, legend_arranged, NA, nonheto_contrast_multiplot, nrow = 7, ncol = 1, heights = c(1, 0.05, 1, 0.05, 0.05, 0.05, 1))
ggview::ggview(flower_visits_multiplot, width = column1.5x, height = column1.5x*1.08, units = "px")
ggsave(file.path(path_name, "Fig4_Camera_visitation.png"), flower_visits_multiplot, dpi = 300, width = column1.5x, height = column1.5x*1.08, units = "px", bg = "white", device = "tiff")
```
...
#legend_arranged <- ggarrange(interaction_plot_legend, NA, nrow = 1, ncol = 2, widths = c(1, 0.75)) #This makes the legend centered under the interaction plots instead of under all plots. However, depending on export size, the first part gets cut off.

pollination_multiplot <- ggarrange(heto_pollination_multiplot, NA, hapa_pollination_multiplot, NA, interaction_plot_legend, nrow = 5, ncol = 1, heights = c(1, 0.05, 1, 0.05, 0.05))
ggview::ggview(pollination_multiplot,  width =  column1.5x*0.66, height = column1.5x*0.75, units = "px")
ggsave(file.path(path_name, "Fig5_Pollination.png"), pollination_multiplot, dpi = 300, width = column1.5x*0.66, height = column1.5x*0.75, units = "px", bg = "white", device = "tiff")
```
...
#Combine
additional_vars_multiplot <- ggarrange(recaps_body_mass_multiplot, NA, marked_birds_multiplot, nrow = 3, ncol = 1, heights = c(1, 0.05, 1), font.label = list(size = label_size, face = "plain"))
ggview::ggview(additional_vars_multiplot, width =  column1.5x*0.66, height = column1.5x*0.75, units = "px")
ggsave(file.path(path_name, "Fig6_Additional_response_variables.png"), additional_vars_multiplot, dpi = 300, width = column1.5x*0.66, height = column1.5x*0.75, units = "px", bg = "white", device = "tiff")
```

Figure SX: Interaction plots for additional response variables
```{r}
#RECAPTURES
#Note: recaptures has control vs. treatment plot (not an interaction plot!) since recaptures were only quantified during 'post' period
control_vs_treatment_plots <- emmeans_for_plots %>%
  left_join(max_values) %>%
  filter(subanalysis == "recaptures") %>%
  arrange(bird_group) %>%
  mutate(control_vs_treatment_plot = pmap(list(ggeffects, yvar, ymin, ymax, ybreak, yaccuracy), ~make_control_vs_treatment_plot(ggeffects_df = ..1, yvar = ..2, ymin = ..3, ymax = ..4, ybreak = ..5, yaccuracy = ..6, text_size = size, point_size = shape_size, line_size = line_width, as_percent = TRUE)))
...
#BODY MASS ONLY
# body_mass_interaction_multiplot <- ggarrange(plotlist = body_mass_interaction_plots$interaction_plot, nrow = 1, ncol = 2, common.legend = TRUE, legend = "bottom", align = "hv", labels = c("(a)", "(b)"), font.label = list(size = label_size, face = "plain"), label.x = 0.17, label.y = 0.96)
# ggview::ggview(body_mass_interaction_multiplot, width =  column1.5x*0.66, height = column1.5x*0.375, units = "px")
```
