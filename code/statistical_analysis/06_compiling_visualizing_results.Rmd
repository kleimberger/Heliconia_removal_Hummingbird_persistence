...
camera_heto_marked_label <- c(expression(atop("Visitation rate: marked birds", "(focal "*italic(Heliconia)*")")))
camera_nonheto_marked_label <- c(expression(atop("Visitation rate: marked birds", "(non-"*italic(Heliconia)*")")))
...
contrast_plot_marked_and_recaps <- contrasts_of_contrasts_df %>%
  ...
  mutate(analysis_id = paste(analysis, subanalysis, sep = "_")) %>%
  make_contrast_plot(contrasts_df = ., xvar = "analysis_id", shading = "below") +
    scale_x_discrete(labels = c("% birds recaptured", camera_heto_marked_label, camera_nonheto_marked_label)) +
    scale_y_continuous(limits = c(0, 3), breaks = c(0, 0.5, 1, 1.5, 2, 3)) +
...
contrast_plot_body_mass <- contrasts_of_contrasts_df %>%
  ...
  mutate(analysis_id = paste(analysis, subanalysis, sep = "_")) %>%
  make_contrast_plot(contrasts_df = ., xvar = "analysis_id", shading = "below") +
    scale_x_discrete(labels = c("Relative body mass")) +
...
contrast_multiplot_hbird_analyses <- ggarrange(contrast_plot_main,
                                               contrast_plot_marked_and_recaps,
                                               common.legend = TRUE, ncol = 1, nrow = 3, align = "hv", legend = "bottom")
...
ggsave(file.path(path_name, "Contrast_multiplot_supp_analyses.png"), contrast_multiplot_supp_analyses, dpi = 300, units = "in", width = 9, height = 6, bg = "white")
