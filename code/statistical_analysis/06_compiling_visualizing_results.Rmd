...
contrasts_cameras_captures <- results_categorical %>%
  ...
  mutate(contrast_pval_check = map(emm, ~as.data.frame(summary(contrast(., interaction = "revpairwise"), type = "response"))))
...
contrasts_telemetry_pollen_tubes <- results_categorical %>%
  ...
  mutate(contrast_pval_check = map(emm_regrid, ~as.data.frame(summary(contrast(., interaction = "revpairwise"), type = "response"))))
...
#P-values reflecting treatment effect contrasts (from emmeans)
#As a sanity check, I will compare them to the p-value associated with interaction term (from model summary)
contrasts_of_contrasts_pvalues_df <- contrasts_of_contrasts %>%
  select(append(col_names, "contrast_pval_check")) %>%
  unnest(contrast_pval_check) %>%
  arrange(model_number) %>%
  mutate_if(is.numeric, ~signif(., digits = 3)) %>%
  select(analysis, subanalysis, bird_group, plant_species, yvar, categorical_p_from_emmeans = p.value)
```
...
#Sanity check: to make sure I calculated the contrast of contrasts correctly, will compare p-value of coefficient of interest to p-value with contrast of interest; they should be pretty close.
sanity_check <- results_supp_table %>%
  rename(categorical_p_from_model = categorical_p) %>%
  left_join(contrasts_of_contrasts_pvalues_df) %>%
  select(analysis, subanalysis, bird_group, plant_species, yvar, starts_with("categorical_p")) %>%
  mutate(p_diff = abs(as.numeric(categorical_p_from_model) - categorical_p_from_emmeans)) %>%
  arrange(desc(p_diff))

####################################################################################
#P-values are pretty close, except for the telemetry models. Will explore further...
telemetry_test_model <- telemetry %>%
  filter(model_type == "categorical" & bird_group == "all_spp" & subanalysis == "all_replicates") %>%
  pull(model)

telemetry_test_model <- telemetry_test_model[[1]]

#P-value from model summary
summary(telemetry_test_model) #p = 0.6101
...
telemetry_test_model_no_interaction <- update(telemetry_test_model, ".~. - control_treatment:exp_phase", data = dataset)
anova(telemetry_test_model, telemetry_test_model_no_interaction) #p = 0.6063

#Seems like model summary p-value is indeed correct, so seems like it's related to emmeans...

#EMMEANS
#Estimated marginal means, back-transformed
emmeans(telemetry_test_model, specs = ~exp_phase:control_treatment, type = "response") 

#Contrasts as risk ratios
emm = emmeans(telemetry_test_model, specs = ~exp_phase:control_treatment)
emm_regrid = regrid(emm, transform = "log")
contrast(emm_regrid, interaction = "revpairwise") %>% summary(type = "response") #p = 0.4954
contrast(emm, interaction = "revpairwise") %>% summary(type = "response") #p = 0.6119 --> OK, get the correct p-value if do not use regrid! Which makes sense, since statistical tests should be run on the scale of the model, rather than the backtransformed data
```
...
heto_visits_marked_contrast_plot <- contrasts_of_contrasts_df %>%
  ...
  make_contrast_plot(contrasts_df = ., xvar = "bird_group", xlabel = "Bird group", text_size = size, point_size = shape_size, line_size = line_width) +
  scale_x_discrete(labels = c("All species", expression(italic(Heliconia)~"specialists"))) +
  #labs(title = "Flower visitation from marked birds: focal"~italic(Heliconia))
...
#FLOWER VISITATION: NON-HETO, MARKED BIRDS ONLY
nonheto_visits_marked_contrast_plot <- contrasts_of_contrasts_df %>%
  filter(analysis == "camera_non-HETO" & subanalysis == "marked_birds") %>%
  make_contrast_plot(contrasts_df = ., xvar = "bird_group", xlabel = "Bird group", text_size = size, point_size = shape_size, line_size = line_width) +
  scale_x_discrete(labels = c("All species", expression(italic(Heliconia)~"specialists"))) +
  #labs(title = "Flower visitation from marked birds: non-"*italic(Heliconia)~"species ("*italic(N)~"= 17)")
...
Figure 3: Captures and Telemetry
```{r, fig.width = column1.5x/300, fig.height = (column1.5x/300)*0.75}
#CAPTURES
...
captures_multiplot <- ggarrange(plotlist = capture_plots, nrow = 1, ncol = 3, legend = "none", align = "hv",
                                labels = c("(a)", "(b)", "(c)"), font.label = list(size = label_size, face = "plain"), label.x = 0.16, label.y = 0.96) %>%
...
ggview::ggview(captures_telemetry_multiplot, width = column1.5x, height = column1.5x*0.75, units = "px")
ggsave(file.path(path_name, "Fig4_Captures_and_Telemetry.png"), captures_telemetry_multiplot, dpi = 300, width = column1.5x, height = column1.5x*0.75, units = "px", bg = "white", device = "tiff")
```

Figure 4: Flower visitation
...
nonheto_visits_multiplot <- ggarrange(plotlist = nonheto_visits_plots, nrow = 1, ncol = 3, legend = "none", align = "hv",
                                 labels = c("(d)", "(e)", "(f)"), font.label = list(size = label_size, face = "plain"), label.x = 0.16, label.y = 0.96) %>%
...
nonheto_contrast_multiplot <- ggarrange(nonheto_contrast_plot, labels = c("(g)"), font.label = list(size = label_size, face = "plain"), label.x = 0.16/3, label.y = 0.96) %>%
  annotate_figure(top = text_grob(expression("Flower visitation: non-"*italic(Heliconia)~"analyzed separately ("*italic(N)~"= 9 species with largest sample sizes)"), size = title_size, face = "plain",  hjust = 0, x = 0.00))

#FLOWER VISITATION: HETO, NON-HETO TOGETHER, + NON-HETO SEPARATELY
flower_visits_multiplot <- ggarrange(heto_visits_multiplot, NULL, nonheto_visits_multiplot, NULL, legend_arranged, NULL, nonheto_contrast_multiplot, nrow = 7, ncol = 1, heights = c(1, 0.05, 1, 0.05, 0.05, 0.05, 1))
ggview::ggview(flower_visits_multiplot, width = column1.5x, height = column1.5x*1.08, units = "px")
ggsave(file.path(path_name, "Fig5_Camera_visitation.png"), flower_visits_multiplot, dpi = 300, width = column1.5x, height = column1.5x*1.08, units = "px", bg = "white", device = "tiff")
```

Figure 5: Pollination success
...
heto_pollination_multiplot <- ggarrange(plotlist = heto_pollination_plots, nrow = 1, ncol = 2, legend = "none", align = "hv",
                                labels = c("(a)", "(b)"), font.label = list(size = label_size, face = "plain"), label.x = 0.17, label.y = 0.96) %>% 
...
hapa_pollination_multiplot <- ggarrange(plotlist = hapa_pollination_plots, nrow = 1, ncol = 2, legend = "none", align = "hv",
                                labels = c("(c)", "(d)"), font.label = list(size = label_size, face = "plain"), label.x = 0.17, label.y = 0.96) %>%
...
ggview::ggview(pollination_multiplot,  width =  column1.5x*0.66, height = column1.5x*0.75, units = "px")
ggsave(file.path(path_name, "Fig6_Pollination.png"), pollination_multiplot, dpi = 300, width = column1.5x*0.66, height = column1.5x*0.75, units = "px", bg = "white", device = "tiff")
```

Figure 6: Additional response variables (recaptures, visitation from individual marked birds, relative body mass)
```{r}
#Upper panel
recaps_body_mass_multiplot <- ggarrange(recaps_contrast_plot, body_mass_contrast_plot, nrow = 1, ncol = 2, align = "hv", labels = c("(a)", "(b)"), font.label = list(size = label_size, face = "plain"), label.x = 0.20, label.y = 0.85)

#Lower panel
marked_birds_multiplot <-  ggarrange(heto_visits_marked_contrast_plot, nonheto_visits_marked_contrast_plot, nrow = 1, ncol = 2, align = "hv", labels = c("(c)", "(d)"), font.label = list(size = label_size, face = "plain"), label.x = 0.16, label.y = 0.88) %>%
    annotate_figure(top = text_grob("Flower visitation from marked birds", size = title_size, face = "plain"))

#Combine
additional_vars_multiplot <- ggarrange(recaps_body_mass_multiplot, NULL, marked_birds_multiplot, nrow = 3, ncol = 1, heights = c(1, 0.05, 1), font.label = list(size = label_size, face = "plain"))
ggview::ggview(additional_vars_multiplot, width =  column1.5x*0.66, height = column1.5x*0.75, units = "px")
ggsave(file.path(path_name, "Fig7_Additional_response_variables.png"), additional_vars_multiplot, dpi = 300, width = column1.5x*0.66, height = column1.5x*0.75, units = "px", bg = "white", device = "tiff")
```

Figure SX: Interaction plots for additional response variables
```{r}
#RECAPTURES
...
additional_vars_interaction_multiplot <- ggarrange(plotlist = additional_vars_supp_plots, nrow = 4, ncol = 2, common.legend = TRUE, legend = "bottom", align = "hv", labels = c("(a)", "(b)", "(c)", "(d)", "(e)", "(f)", "(g)", "(h)"), font.label = list(size = label_size, face = "plain"), label.x = 0.17, label.y = 0.96)

additional_vars_interaction_multiplot
ggview::ggview(additional_vars_interaction_multiplot, width =  column1.5x*0.66, height = column1.5x*1.33, units = "px")
ggsave(file.path(path_name, "FigSX_Additional_response_variables_interactions.png"), additional_vars_interaction_multiplot, dpi = 300, width = column1.5x*0.66, height = column1.5x*1.33, units = "px", bg = "white", device = "tiff")
```
