...
captures_all_years <- read.csv("../../data/import/Hummingbird_captures_2010-2018_20220713.csv") %>%
...
recaptures_experiment_measurements <- recaptures_experiment %>%
  ...
  select(year, patch, year_patch, control_treatment, exp_phase, bird_id, species, mass, wing_length, notes) %>%
...
data_mass_wing_regressions <- captures_all_years %>%
  ...
  select(year, patch, exp_phase, species, bird_id, log_mass, log_wing_length, experiment_bird)
```
...
mass_wing_regressions <- recaptures_experiment_for_analysis %>%
  distinct(species) %>%
  left_join(bird_species_names) %>%
  mutate(data = map(species, ~subset_data(dataset = data_mass_wing_regressions, bird_species = .))) %>% 
...
outliers <- mass_wing_regressions %>%
  ...
  mutate(plot_outlier = pmap(list(data_augment_outliers, common_name), ~make_plot_outlier_id(dataset = ..1, bird_species = ..2, group_id = "cooksd_outlier")))
...
mass_wing_regressions_without_outliers <- recaptures_experiment_for_analysis %>%
  distinct(species) %>%
  left_join(bird_species_names) %>%
  mutate(data = map(species, ~subset_data(dataset = data_mass_wing_regressions_without_outliers, bird_species = .))) %>% 
  mutate(num_rows = map(data, function(.){unlist(dim(.)[[1]])})) %>%
  filter(num_rows > 30) %>% #Remove species that don't have > 30 observations for regression
  mutate(mass_wing_model = map(data, create_model)) %>%
  ...
  mutate(data_augment = pmap(list(data, mass_wing_model), ~augment_data(dataset = ..1, model_object = ..2))) %>%
  mutate(plot_recaps = pmap(list(data_augment, common_name, mass_wing_intercept, mass_wing_slope), ~make_plot(dataset = ..1, bird_species = ..2, intercept = ..3, slope = ..4, group_id = "experiment_bird"))) %>%
...
```{r}
recaptures_relative_mass_for_analysis <- bind_rows(mass_wing_regressions_without_outliers$data_augment) %>%
...
data_for_analysis <- recaptures_experiment_for_analysis %>%
  left_join(recaptures_relative_mass_for_analysis)
```
...
write.csv(data_for_analysis, "../../data/export/for_analysis/Relative_body_mass_for_analysis.csv")
