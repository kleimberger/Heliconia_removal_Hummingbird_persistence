...
```{r}
experiment_birds_relative_mass_for_analysis <- bind_rows(mass_wing_regressions_without_outliers$data_augment) %>%
...
data_for_analysis <- recaptures_experiment_for_analysis %>%
  left_join(experiment_birds_relative_mass_for_analysis) %>%
  distinct() #For some reason this join is producing 8 rows/row (??)
```
