...
weights <- weights_all_species %>%
  ...
  select(bird_group, rank, weight, plant_family, plant_scientific_name, plant_species, everything())

#Make weights table for Supplement
weights_for_supp_data <- weights %>%
  select(-c(max_rate, rank, flowers, hours, sightings_time)) %>%
...
resource_data_sum_calories_subset <- resource_data_to_summarize_with_weights %>%
  ...
  group_by(bird_group, count_estimate_high_low, year, patch, year_patch, control_treatment, removed_remaining, heto_nonheto) %>%
  summarise(num_calories = sum(calories_per_plant, na.rm = TRUE)) %>%
  ungroup() %>%
  mutate(estimate_name = "med")
...
percentage_calories_removed_tailored_supp <- percentage_calories_removed_tailored %>%
  select(estimate_name, count_estimate_high_low, bird_group, year, patch, control_treatment, percentage_removed) %>% 
  filter(count_estimate_high_low == "high") %>%
...
## Step 12: Visualize weights and % calories removed
...
Make barplots of relative sighting rates (weights)
```{r step12b}
...
Make boxplots of %  calories removed
```{r step12c}
plot_theme = 
...
data_for_boxplots <- percentage_calories_removed_tailored %>%
  ...
  mutate(estimate_name = factor(estimate_name, levels = c("low", "med", "high"), labels = c("Low estimate", "Medium estimate", "High estimate")))
...
percentage_calories_removed_boxplots <-  data_for_boxplots %>%
  ggplot(data = ., aes(x = bird_group, y = percentage_removed)) +
    ...
    geom_boxplot(position = position_nudge(x = 0, y = 0), width = 0.15, outlier.shape = NA, alpha = 1, fill = "grey80", size = line_width) +
    ...
    stat_summary(fun = mean, geom = "point", shape = 23, size = shape_size, color = "#BA0022", fill = "#BA0022") +
...
Combine weights and % calories removed boxplots into one multi-panel figure.
```{r step12d}
#Vertical
...
ggview::ggview(weights_and_percent_multiplot, width = column1.5x, height = column1.5x*2, units = "px")

ggsave(file.path(path_name, "Fig3_Weights_Percentage_calories.png"), weights_and_percent_multiplot, dpi = 300, width = column1.5x, height = column1.5x*2, units = "px", bg = "white", device = "tiff")
```

## Step 13: What plant species were present in each replicate/site?
...
plant_occurrence_per_replicate <- resource_data_to_summarize %>%
  distinct(year, patch, year_patch, plant_family, plant_scientific_name, plant_species) %>%
...
## Step 14: What plant species provided the most calories in each replicate?
...
ggview::ggview(heto_rank_barplot, width = column1.5x, height = column1.5x/2, units = "px")
...
ggview::ggview(top_ranked_species_barplot, width = column1.5x, height = column1.5x/2, units = "px")
```
...
write.csv(weights_for_supp_data, "../../data/export/supp_tables/Weights_for_resource_estimate.csv", row.names = FALSE)
write.csv(percentage_calories_removed_tailored_supp, "../../data/export/supp_tables/Proportion_calories_removed.csv", row.names = FALSE)
write.csv(percentage_calories_removed_tailored_sum, "../../data/export/supp_tables/Proportion_calories_removed_sum.csv", row.names = FALSE)
