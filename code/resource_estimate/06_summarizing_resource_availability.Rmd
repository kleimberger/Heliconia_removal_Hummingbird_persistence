...
percentage_calories_removed_tailored <- resource_data_sum_calories %>%
  ...
  ungroup() %>%
  pivot_wider(names_from = removed_remaining, values_from = num_calories) %>%
  rename(heto_calories_removed = removed, heto_nonheto_calories = remaining) %>%
  mutate(percentage_removed = (heto_calories_removed/(heto_calories_removed + heto_nonheto_calories)) * 100) %>%
  ...
  select(estimate_name:control_treatment, heto_calories_removed, heto_nonheto_calories, percentage_removed, -count_estimate_high_low) %>%
...
percentage_calories_removed_tailored <- percentage_calories_removed_tailored %>%
  ...
  mutate(across(c(heto_calories_removed, heto_nonheto_calories, nonheto_calories, nonheto_calories_per_ha), ~round(., digits = 0)))
```
