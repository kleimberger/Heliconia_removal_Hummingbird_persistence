...
resource_data <- read.csv("../../data/import/Resource_counts_2016-2018_20200713.csv") %>%
...
I can still include these species in the calorie estimate by substituting a closely related species. To do this, I will create a new column called 'species_code_for_calories'.  *Remember, this is just for the resource/calorie estimate! Use original 'species_code' to calculate other variables, e.g., species richness.*
...
resource_data02 <- resource_data %>%
  mutate(species_code_for_calories = case_when(species_code == "UNKN-PASSIFLORA" ~ "PACO",
                                               species_code == "UNKN-HELICONIA" ~ "HETO",
                                               species_code == "UNKN-COSTUS" ~ "COLA",
                                               species_code == "UNKN-HIBISCUS" ~ "AMAP",
                                               species_code == "UNKN-BROMELIA" ~ "GUZA", 
                                               species_code == "UNKN-PAV-MOR-PEL" ~ "PAV-MOR",
                                               TRUE ~ as.character(species_code)))%>%
  select(year:family, species_code_for_calories, count_unit, count_unit_status, everything())
```
...
*Note: need to use 'species_code_for_calories' so that unknown/approximated species get assigned count units*
...
resource_data03 <-resource_data02 %>%
  ...
  mutate(count_unit_source = ifelse(count_unit_status == "unknown" & grepl(pattern = "HE", species_code_for_calories), "Counting protocol for Heliconias", ""),
         count_unit_status = ifelse(count_unit_status == "unknown" & grepl(pattern = "HE", species_code_for_calories), "assumed", count_unit_status),
         count_unit = ifelse((is.na(count_unit) | count_unit == "inflor") & grepl(pattern = "HE", species_code_for_calories), "bract", as.character(count_unit))) %>%
  
  #Assign "flower" to species presumably counted as flowers
  mutate(count_unit_source = ifelse(is.na(count_unit) & (species_code_for_calories %in% species_counted_as_flowers), "No countable inflorescence", count_unit_source),
         count_unit_status = ifelse(is.na(count_unit) & (species_code_for_calories %in% species_counted_as_flowers), "assumed", count_unit_status),
         count_unit = ifelse(is.na(count_unit) & (species_code_for_calories %in% species_counted_as_flowers), "flower", as.character(count_unit))) %>%
  
  #Assign "inflor" to species presumably counted as inflorescences
  mutate(count_unit_source = ifelse(is.na(count_unit) & (species_code_for_calories %in% species_counted_as_inflor), "No countable flower", count_unit_source),
         count_unit_status = ifelse(is.na(count_unit) & (species_code_for_calories %in% species_counted_as_inflor), "assumed", count_unit_status),
         count_unit = ifelse(is.na(count_unit) & (species_code_for_calories == "GUMA"), "inflor", as.character(count_unit))) %>%
...
some_known <- resource_data03 %>%
  group_by(species_code, species_name, count_unit_status) %>%
...
density_plot_data <- resource_data03 %>%
          ...
          filter(species_code %in% some_known$species_code) %>% 
...
density_plots <- ggplot(data = density_plot_data, aes(x = log(num_other_resources), fill = count_info)) +
            ...
            facet_wrap(. ~ species_code, nrow = NULL, ncol = NULL, scales = "free") +
...
resource_data04 <- resource_data03 %>%
  mutate(count_unit_source = ifelse(is.na(count_unit) & grepl(pattern = "GINGY|MARA|MARA-VER", species_code_for_calories), "Known vs. unknown distributions", count_unit_source),
         count_unit_status = ifelse(is.na(count_unit) & grepl(pattern = "GINGY|MARA|MARA-VER", species_code_for_calories), "assumed", count_unit_status),
         count_unit = ifelse(is.na(count_unit) & grepl(pattern = "GINGY|MARA|MARA-VER", species_code_for_calories), "inflor", as.character(count_unit)))
```
...
still_unknown <- resource_data04 %>%
  ...
  group_by(species_code, species_name, species_code_for_calories) %>%
  summarise(num_rows = n()) %>%
  ungroup() %>%
  arrange(desc(num_rows)) 

#How many species still have unknown count units? Filtered to species that could be identified to genus
filter(still_unknown, !is.na(species_name)) %>% pull(species_name) %>% length()

#How many rows (plants) have unknown count units?
unknown_plants <- filter(still_unknown, !is.na(species_name)) %>% 
...
## Step 7: Export data
```{r step7}
write.csv(resource_data07, "../../data/export/intermediate/Resource_counts_2016-2018_with_count_units.csv") 
```
