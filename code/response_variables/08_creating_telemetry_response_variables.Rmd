...
#Bird info
birds_tagged <- read.csv("../../data/import/Telemetry_birds.csv") %>%
  select(year, patch = site, species, sex, frequency = frequency_adjusted, bird_notes = notes) %>%
...
birds_with_data_reliability_issues <- birds_tagged_time_in_patch %>%
  ...
  select(species, sex, frequency, exp_phase, time_in_patch) %>%
  group_by(species, sex, frequency, exp_phase) %>%
...
birds_never_in_patch <- birds_tagged_time_in_patch %>%
  ...
  group_by(frequency, species, sex) %>%
  summarise(time_in_patch = sum(time_in_patch)) %>%
...
birds_never_in_patch_during_pre <- birds_tagged_time_in_patch %>%
  ...
  group_by(frequency, species, sex, exp_phase) %>%
  summarise(time_in_patch = sum(time_in_patch)) %>%
...
birds_with_outcomes <- birds_tagged %>%
  ...
  mutate(sex = ifelse(sex == "UF" | sex == "UM", "U", sex)) %>% #unknown female and unknown male = unknown sex
  distinct(outcome, species, sex, frequency)

#Radio tracking outcomes, by species/sex
outcomes_by_species_sex <- birds_with_outcomes %>%
  group_by(outcome, species, sex) %>%
...
prop_time_for_analysis <- time_in_patch_for_analysis %>%
  group_by(year, patch, control_treatment, frequency, species, sex, exp_phase) %>%
...
locations_per_bird <- telem_data_for_analysis %>%
  ...
  group_by(year, patch, control_treatment, frequency, species, sex) %>%
...
dates_per_bird <- time_in_patch_for_analysis %>%
  group_by(year, patch, control_treatment, species, sex, frequency) %>%
...
## Step 8: Calculate mean time spent at location (for analysis of activity level/torpor)
...
time_at_location_birds_complete <- time_at_location_sum %>%
  distinct(year, patch, control_treatment, frequency, species, sex, exp_phase, num_locations) %>%
...
## Step 9: Export data
```{r, step9}
write.csv(prop_time_for_analysis, "../../data/export/for_analysis/Telemetry_proportion_time_in_patch.csv")
write.csv(telem_data_for_analysis, "../../data/export/intermediate/Telemetry_data_filtered_for_analysis.csv")

#Radio-tracking outcomes for Supp material
write.csv(outcomes_by_species_sex, "../../data/export/supp_tables/Telemetry_outcomes_by_species_and_sex.csv")
write.csv(outcomes_by_species, "../../data/export/supp_tables/Telemetry_outcomes_by_species.csv")
write.csv(outcomes, "../../data/export/supp_tables/Telemetry_outcomes.csv")
```