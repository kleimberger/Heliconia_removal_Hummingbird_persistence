...
captures <- read.csv("../../data/import/Hummingbird_captures_2010-2018_20220713.csv") %>%
  ...
  mutate(bird_id = ifelse(is.na(band_number), paste(year, patch, species, colors, sep = "_"), paste(year, patch, species, band_number, sep = "_"))) %>%
...
#Scientific names
bird_species_names <- read.csv("../../data/import/Hummingbird_codes_species_list.csv") %>%
  rename(species = code)
...
recaps_within_day <- captures %>%
  distinct(patch, date, species, band_number) %>%
  filter(!is.na(band_number)) %>% #NA for birds not banded
  group_by(date, species, band_number) %>%
...
recaps_across_sites <- captures %>%
  distinct(patch, species, band_number) %>%
  filter(!is.na(band_number)) %>% #NA for birds not banded
  group_by(species, band_number) %>%
...
recaps_across_years <- captures %>%
  distinct(year, species, band_number) %>% #Don't bother with color marks; will have worn off.
  filter(!is.na(band_number)) %>% #NA for birds not banded
  group_by(species, band_number) %>%
...
recaps_within_experiment <-  captures %>%
  distinct(year, patch, exp_phase, species, bird_id, band_number, colors) %>% #Not all birds banded, so use colors marks as well.
  group_by(year, patch, species, bird_id, band_number, colors) %>%
  summarise(num_captures = n()) %>%
  filter(num_captures > 1)
...
num_birds_greh_visa <- captures %>% 
  filter(species == "GREH" | species == "VISA") %>% 
  group_by(year, patch, control_treatment, date, exp_phase) %>% 
  summarise(num_birds = n()) %>% 
  ungroup() %>% 
  right_join(net_hours) %>% #Add info about net hours. Right join so can fill in missing data with zeroes
...
DECISION: For space use variables (captures + telemetry), remove replicates without confirmed presence of hummingbird(s) in focal area during 'pre' period.
```{r}
no_birds_in_pre_replicates <- capture_rates %>%
  filter(exp_phase == "capture_1" & num_birds == 0) %>%
  mutate(birds_in_pre = "no")
...
capture_rates_birds_in_pre_only <- capture_rates %>%
  left_join(no_birds_in_pre_replicates) %>%
  filter(is.na(birds_in_pre)) %>%
  select(-birds_in_pre)
```
...
recaptures <- post_birds %>%
  ...
  select(year, patch, control_treatment, bird_id, species)
...
#CAPTURES
#For main analysis
write.csv(capture_rates_birds_in_pre_only, "../../data/export/for_analysis/Capture_rates.csv")

#For Supp table
write.csv(capture_summary_table, "../../data/export/supp_tables/Captured_species_per_site_summary_table.csv")

#RECAPTURES
#For main analysis
write.csv(recapture_rates, "../../data/export/for_analysis/Recapture_rates.csv")

#For analysis of pre-post changes in body condition
write.csv(recaptures, "../../data/export/intermediate/Birds_recaptured_post.csv")

#For potential future analysis of whether individual attributes influence recapture (0/1 for each bird)
write.csv(pre_birds_with_recap_status, "../../data/export/intermediate/Birds_captured_pre_with_recap_status.csv") 
```
